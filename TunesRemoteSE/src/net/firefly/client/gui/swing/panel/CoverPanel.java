...
public class CoverPanel extends JPanel implements PlayerStatusChangedEventListener, ContextResetEventListener {
	...
	protected void initialize() {
		...
		cover = new JLabel();
		cover.setBorder(new EmptyBorder((MARGIN_TOP ? COVER_MARGIN : 0), COVER_MARGIN, 0, COVER_MARGIN));
	...
	private void displayCover(BufferedImage bufferedImage) {
		...
		int coverWidth = ((getWidth()!=0)?getWidth():150) - (2 * COVER_MARGIN);
		int coverHeight = coverWidth * originalHeight / originalWidth;
		if (originalHeight > originalWidth) {
			coverHeight = ((getWidth()!=0)?getWidth():150) - (2 * COVER_MARGIN);
			coverWidth = coverHeight * originalWidth / originalHeight;
		}
		BufferedImage tmp = ImageTools.getSmoothScaledInstance(bufferedImage, coverWidth, coverHeight);
		tmp = ImageTools.createReflectBottom(tmp, SONG_INFO_HEIGHT);
		tmp = overlaySongInfo(context.getPlayer().getPlayingSong().getSong(), tmp, coverHeight);
		cover.setIcon(new ImageIcon(tmp));
	}

	private BufferedImage overlaySongInfo(Song s, BufferedImage cover, int startOffset) {
		if (s == null) {
			return cover;
		}
		...
		BufferedImage result = new BufferedImage(finalCoverWidth, cover.getHeight(), cover.getType());
		...
		// -- draw centered cover
		g.drawImage(cover, finalCoverWidth / 2 - cover.getWidth() / 2, 0, cover.getWidth(), cover.getHeight(), null);

		// -- draw artist name (centered)
		g.setFont(this.getFont().deriveFont(Font.BOLD));
		TextLayout tl = new TextLayout(artist, g.getFont(), g.getFontRenderContext());
		Rectangle2D artistBounds = tl.getBounds();
		double x = (finalCoverWidth - artistBounds.getWidth()) / 2 - artistBounds.getX();
		...
		x = (finalCoverWidth - albumYearBounds.getWidth()) / 2 - albumYearBounds.getX();
		y = startOffset + 29;
		tl.draw(g, (float) x, (float) y);
		...
		if (title != null && title.length() > 0) {
			...
			x = (finalCoverWidth - titleBounds.getWidth()) / 2 - titleBounds.getX();
			y = startOffset + 43;
			tl.draw(g, (float) x, (float) y);
		}
		g.dispose();
